...
exports.getHomeFouseMap = (req, res) => {
  ...
  db.query(sqlStr, res.userId, (err, results) => {
    if (err) return res.cc(err)
    // 当用户未设置自定义轮播图，将默认图片发布
    if (results instanceof Array && results.length === 0) {
      let sqlStr1 = 'select id, imgUrl, imgText from mapTable where userId = -1'
      db.query(sqlStr1, (err1, results1) => {
        if (err1) return res.cc(err1)
        if (results1.length === 0) return res.cc("获取图片失败")
        res.send({
          status: 1,
          message: '成功',
          queryData: results1
...
exports.releaseArticle = (req, res) => {
  ...
  db.query(sqlStr, [res.userId, addData.article, addData.title, newTime], (err, results) => {
    if (err) return res.cc(err)
...
exports.updataArticle = (req, res) => {
  ...
  db.query(sqlStr, [info.article, info.title, parseInt(info.articleId), res.userId], (err, results) => {
    if (err) return res.cc(err)
...
exports.reomveArticle = (req, res) => {
  ...
  db.query(sqlStr, [res.userId, parseInt(userInfo.articleId)], (err, results) => {
    if (err) return res.cc(err)
...
exports.getArticleList = (req, res) => {
  ...
  if (info.page || info.size) {
    ...
    db.query(sqlStr, [res.userId, (parseInt(info.page) * parseInt(info.size)), parseInt(info.size)], (err, results) => {
      if (err) return res.cc(err)
...
exports.releaseWorks = (req, res) => {
  ...
  let newTime = formatTime.format(new Date(), 'YYYY-MM-DD HH:mm:ss')
  db.query(sqlStr, [info.title, parseInt(info.platform), res.userId, info.wordsAddress, newTime], (err, results) => {
    if (err) return res.cc(err)
...
exports.removeWorks = (req, res) => {
  ...
  db.query(sqlStr, [res.userId, parseInt(info.id)], (err, results) => {
    if (err) return res.cc(err)
    if (results.affectedRows !== 1) return res.cc('删除文章失败')
    res.send({
      status: 1,
